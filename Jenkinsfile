pipeline {
  ...
  stages {
    ...
    stage('Deploy') {
      steps {
        sh "ssh rigibon@192.168.0.211 'kubectl set image deployment/frontend-deployment frontend-prueba=${DOCKERHUB_CREDENTIALS_USR}/frontend-prueba:${env.BUILD_NUMBER}'"
      }
    }
  }
}
